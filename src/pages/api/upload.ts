...
type UploadResponseFile = UploadFile & { alt: string; uploadUrl: string; publicUrl: string };
...
export default async function uploadEndpoint(
  ...
  try {
    ...
    if (!Array.isArray(files) || files.length < 1 || files.length > MAX_FILES || !files.every(isValidFile)) {
      res.status(400).json({ error: 'Invalid media metadata' });
      return;
    }
    const uploadedFiles = await Promise.all(files.map(async ({ id, name, type }) => {
