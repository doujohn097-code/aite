...
export function PostComments({
  ...
  // Long press to delete (mobile) — mirrors reels comments behaviour
  ...
  const allComments = useMemo(() => {
    ...
    return [...pendingOptimistic, ...serverList].sort(
      (a, b) => toMillis(b.createdAt) - toMillis(a.createdAt)
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [rawComments, optimisticComments]);

  // Group into root comments and threaded replies
    ...
    <>
      ...
      >
        ...
        <div className='flex items-center gap-2 border-y border-light-border px-4 py-3 dark:border-dark-border'>
          ...
          {!!allComments.length && (
            <span
              className='rounded-full bg-light-line-reply/50 px-2 py-0.5 text-xs font-semibold
                         text-light-secondary dark:bg-dark-line-reply/50 dark:text-dark-secondary'
            >
              {allComments.length}
            </span>
          )}
        ...
        <div className='flex flex-1 flex-col gap-4 px-4 py-4'>
          {loading && !allComments.length ? (
            ...
            <div className='flex flex-col items-center justify-center gap-2 py-12 text-center text-light-secondary dark:text-dark-secondary'>
              <div
                className='flex h-14 w-14 items-center justify-center rounded-full bg-light-line-reply/30
                           text-light-secondary dark:bg-dark-line-reply/30 dark:text-dark-secondary'
              >
                <HeroIcon
                  className='h-7 w-7'
            ...
            rootComments.map((item: TweetWithUser) => {
              ...
              const isPostOwner = user?.id === ownerId;
              ...
              return (
                ...
                >
                    ...
                    <div className='flex flex-1 flex-col'>
                      ...
                      {hasReplies && (
                        <button
                          ...
                          className='mt-2.5 flex select-none items-center gap-2 text-xs font-bold
                                     text-main-accent-text transition hover:brightness-110'
                        >
                  ...
                  {hasReplies && isExpanded && (
                    <div
                      className='mt-2.5 flex flex-col gap-3 border-dashed border-main-accent/30 py-1 [border-inline-start-width:2px]
                                 [margin-inline-start:1.25rem] [padding-inline-start:0.875rem]'
                    >
                      {replies.map((reply: TweetWithUser) => {
                        const replyLikes =
                          optimisticLikes[reply.id] ?? reply.userLikes ?? [];
                        const isReplyLiked = replyLikes.includes(
                          user?.id ?? ''
                        );
                        const isReplyAuthor = user?.id === reply.createdBy;
                        const canDeleteReply = isReplyAuthor || isPostOwner;
                        ...
                        return (
                          <div
                            ...
                            onTouchStart={() =>
                              startLongPress(reply.id, true, canDeleteReply)
                            }
                            onTouchEnd={cancelLongPress}
                            onTouchMove={cancelLongPress}
                            onMouseDown={() =>
          ...
          <AnimatePresence>
            {replyingTo && (
              <motion.div
                ...
                className='flex items-center justify-between overflow-hidden bg-main-accent/10 px-4 py-2 text-xs text-main-accent-text'
              >
