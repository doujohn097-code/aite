...
if (rs.code !== 200)
  throw new Error(`ruleset create: ${rs.code} ${JSON.stringify(rs.data)}`);
...
if (rel.code !== 200)
  throw new Error(`release patch: ${rel.code} ${JSON.stringify(rel.data)}`);
...
for (const idx of wanted) {
  ...
  if (r.code === 200) {
    ...
    skipped++;
    console.warn(
      `index skipped (IAM lacks datastore.indexes.create): ${cg} — create manually from Firebase Console once`
    );
  } else {
    console.warn(
      `index issue ${cg}: ${r.code} ${JSON.stringify(r.data).slice(0, 160)}`
    );
  }
}
...
if (skipped > 0) {
  console.log(
    'NOTE: skipped indexes are optional for auth; queries will still work via fallback.'
  );
}
